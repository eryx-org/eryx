...
**Important UX requirement:** Host functions should be callable with natural Python syntax:
```python
# Instead of: invoke("my_function", '{"arg1": 1, "arg2": 2}')
# Users should be able to write:
my_function(arg1=1, arg2=2)
```

This means we need to:
1. Query `list-callbacks` at initialization to discover available host functions
2. Dynamically create Python wrapper functions for each callback
3. Each wrapper serializes `**kwargs` to JSON and calls `invoke(name, json)`
4. Parse the JSON result and return it to Python

**Implementation approach:**
```python
# Injected into __main__ at initialization
import json as _json

def _eryx_make_callback(name):
    def callback(**kwargs):
        args_json = _json.dumps(kwargs)
        result_json = _eryx_invoke(name, args_json)  # C FFI call
        return _json.loads(result_json)
    return callback

# For each callback from list-callbacks:
# globals()[callback_name] = _eryx_make_callback(callback_name)
```
